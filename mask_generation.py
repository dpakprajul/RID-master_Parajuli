...
import pandas as pd
import shapely
from shapely.geometry import MultiPolygon, Polygon, Point, box, MultiLineString
...
def import_vector_labels(file_vector_labels, mask_generation_case, label_classes):
    ...
    if mask_generation_case == 'superstructures':
    ...
    elif mask_generation_case == 'segments_try':
        df_labels = df_labels[df_labels.segment.notna()]
        class_data = [label_classes for label_classes in df_labels.type]
        ...
        df_labels.insert(df_labels.shape[1], 'class_type', class_data, True) #insert class_type of label to dataframe
        label_geoms = list(map(wkt_to_shape, [segment for segment in df_labels.segment]))
    elif mask_generation_case == 'segments_gable':
        df_labels = df_labels[df_labels.gutter.notna()]
        class_data = [label_classes for label_classes in df_labels.type]
        df_labels.insert(df_labels.shape[1], 'class_type', class_data, True) #insert class_type of label to dataframe
        label_geoms = list(map(wkt_to_shape, [segment for segment in df_labels.gutter]))
    elif mask_generation_case == 'segments_gutter':
        df_labels = df_labels[df_labels.gables.notna()]
        class_data = [label_classes for label_classes in df_labels.type]
        df_labels.insert(df_labels.shape[1], 'class_type', class_data, True) #insert class_type of label to dataframe
        label_geoms = list(map(wkt_to_shape, [segment for segment in df_labels.gables]))
    ...
    elif mask_generation_case == 'roofline':
        df_labels = df_labels[df_labels.segment.notna()]
        class_data = [label_classes for label_classes in df_labels.type]
        df_labels.insert(df_labels.shape[1], 'class_type', class_data, True) #insert class_type of label to dataframe
        label_geoms = list(map(wkt_to_shape, [segment for segment in df_labels.segment]))
...
def vector_labels_to_masks(file_vector_labels, dir_binary_masks, mask_generation_case, label_classes, gdf_images, filter=True):
    ...
    for count, gdf_image in enumerate(gdf_images.iloc):
        ...
        gdf_labels_in_image['class_type']
        gdf_labels_in_image = gdf_labels_in_image.loc[:, ~gdf_labels_in_image.columns.duplicated(keep='last')]
        #gdf_labels_in_image['class_type']
        ###############################################
        #### Create binary masks
        ...
        for label_id, label_class in label_classes.items():
            ...
            #gdf_labels_in_image = gdf_labels_in_image.reset_index()
...
def vector_labels_to_masks_try(file_vector_labels, dir_binary_masks, mask_generation_case, label_classes, gdf_images, filter=False):
    # Import labels as vector data
    gdf_labels = import_vector_labels(file_vector_labels, mask_generation_case, label_classes)
                ...
                for label in gdf_label_class.geometry.iloc:
                    ...
                    if type(label) == MultiPolygon: # make sure, label is of type MultiPolygon
                        for label_polygon in label:
                            ...
                            # Update image with label
                            image = cv2.fillPoly(image, pts=[polygon_points], color=(int(label_id)), lineType=cv2.LINE_AA)
                            # cv2.imshow("filledPolygon", image) # activate this line for debugging
    ...
    return gdf_labels
#Deepak coded
def vector_labels_to_masks_gable(file_vector_labels, dir_binary_masks, mask_generation_case, label_classes, gdf_images, filter=False):
    # Import labels as vector data
    gdf_labels = import_vector_labels(file_vector_labels, mask_generation_case, label_classes)
                ...
                for label in gdf_label_class.geometry.iloc:
                    ...
                    if type(label) == MultiPolygon: # make sure, label is of type MultiPolygon
                        for label_polygon in label:
                            ...
                            # Update image with label
                            image = cv2.fillPoly(image, pts=[polygon_points], color=(int(label_id)), thickness=4)
                            # cv2.imshow("filledPolygon", image) # activate this line for debugging

                    elif type(label) == MultiLineString:
                        for label_polygon in label:
                            polygon_points = convert_between_latlon_and_pixel(label_polygon, image_bbox_px,
                                                                          image_bbox, case='latlon_to_px')
                            polygon_points = np.int32(np.array(polygon_points))
                            image = cv2.polylines(image, pts=[polygon_points], isClosed=True, color=(int(label_id)), thickness=3)
                        #print('Label is passed as a Polygon. This case is not covered, yet. ')
                    else:
                        print('''Label not passed as a Polygon or MultiPolygon. 
                        This case is not covered, yet. Check how that happened''')
...
def vector_labels_to_masks_gutter(file_vector_labels, dir_binary_masks, mask_generation_case, label_classes, gdf_images, filter=False):
    # Import labels as vector data
    gdf_labels = import_vector_labels(file_vector_labels, mask_generation_case, label_classes)
                ...
                for label in gdf_label_class.geometry.iloc:
                    ...
                    if type(label) == MultiPolygon: # make sure, label is of type MultiPolygon
                        for label_polygon in label:
                            ...
                            # Update image with label
                            image = cv2.fillPoly(image, pts=[polygon_points], color=(int(label_id)),thickness=5, lineType=cv2.LINE_AA)
                            # cv2.imshow("filledPolygon", image) # activate this line for debugging
                        ...
                        for label_polygon in label:
                            polygon_points = convert_between_latlon_and_pixel(label_polygon, image_bbox_px,
                                                                          image_bbox, case='latlon_to_px')
                            polygon_points = np.int32(np.array(polygon_points))
                            image = cv2.polylines(image, pts=[polygon_points], isClosed=True, color=(int(label_id)), thickness=5, lineType=cv2.LINE_AA)
                        #print('Label is passed as a Polygon. This case is not covered, yet. ')
                    else:
                        print('''Label not passed as a Polygon or MultiPolygon. 
                        This case is not covered, yet. Check how that happened''')
...
def vector_labels_to_masks_roofline(file_vector_labels, dir_binary_masks, mask_generation_case, label_classes, gdf_images, filter=False):
    # Import labels as vector data
    gdf_labels = import_vector_labels(file_vector_labels, mask_generation_case, label_classes)
        ...
        # else: all images and masks are in one folder. train, val and test are loaded using filenames from .txt file
            ...
            for j, data_set_type in enumerate(data_set_types):
                ...
                filepath = os.path.join(dir_target_segmentation_model_data,
                                        (data_set_type + '_filenames_' + str(i + 1) + '_6_classes.csv'))
