...
def geotif_to_png(dir_geotif, dir_png):
    ...
    files_geotif = [geotif[:-4]
                    for geotif in os.listdir(dir_geotif) if geotif[-4:] == '.tif']
    files_png = [png[:-4] for png in os.listdir(dir_png) if png[-4:] == '.png']

    missing_pngs_list = [
        geotif for geotif in files_geotif if geotif not in files_png]

    # open geotif and save as png
    for i, img in enumerate(missing_pngs_list):
        progress_string = get_progress_string(
            round(i / len(missing_pngs_list), 2))
...
def convert_lonlat_to_metric(obj):
    # long lat: coordinates need to be in Long Lat, instead of Lat Long
...
def convert_between_latlon_and_pixel(geom, image_px, image_latlon, case='latlon_to_px'):
    ...
    point_list = []
    for x, y in coords:
        x_new = itx_min + ((x - iox_min) / (iox_max - iox_min)
                           * (itx_max - itx_min))
        y_new = ity_min + ((ioy_max - y) / (ioy_max - ioy_min)
                           * (ity_max - ity_min))
...
def get_image_gdf_in_directory(DIR_IMAGES_GEOTIFF, save_to_png_path=[]):
    image_id_list = [id[:-4]
                     for id in os.listdir(DIR_IMAGES_GEOTIFF) if id[-4:] == '.tif']
    ...
    gdf_images = gpd.GeoDataFrame(
        {'id': image_id_list, 'geometry': image_bbox_list})
...
def get_wartenberg_boundary():
    ...
    if not os.path.isfile(file_path_boundary):
        ...
        r = overpy.Overpass().query(
            '[out:json]; relation(934742); (._;>;); out;')
        ls = []
        for w in r.ways:
            ls.append(LineString(
                [Point(float(node.lon), float(node.lat)) for node in w.nodes]))
        # polygonize seperate ways into one multipolygon
        # (first way is a polygon already, other ways are linestrings)
        pol1 = shapely.ops.polygonize_full(ls[0])[0][0]
        pol2 = shapely.ops.polygonize_full(ls[1:])[0][0]
        mp_Wartenberg = MultiPolygon([pol1, pol2])
        # write to geodataframe and save
        gdf_Wartenberg_boundary = gpd.GeoDataFrame(
            {'name': 'Wartenberg boundary', 'geometry': [mp_Wartenberg]})
...
def prediction_raster_to_vector(prediction_mask, mask_id, image_bbox, CLASSES, IMAGE_SHAPE):
    ...
    label_type_list = []
    geometry_list = []
 ##note from sebastian: remove background classes
...
def prediction_raster_to_vector1(prediction_mask, mask_id, image_bbox, CLASSES, IMAGE_SHAPE):
    """
    Takes the roof image as input and outputs the supersturctures polygon
    Parameters
    ----------
 ...
 ##note from sebastian: remove background classes
    for i, class_id in enumerate(CLASSES):
        #prediction = np.zeros(IMAGE_SHAPE)
        #prediction[prediction_mask == class_id] = class_id
        prediction = prediction_mask.astype(np.uint8)

        if np.sum(prediction) > 0:
            # get the contours from the mask
            contours, _ = cv2.findContours(
                prediction, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
#cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
            # Convert the contours list to a Numpy array
            contours = np.array(contours, dtype=object)
