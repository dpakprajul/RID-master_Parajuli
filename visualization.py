...
class TUM_CI_colors:
    ...
    # sem_seg_map
    colors = list([TUM_blue, lighter_blue, orange])
...
def confusion_matrix_heatmap_TUM_CI(data, data_label_list, save_path, image_size='small'):
    ...
    ax.set_xlabel("prediction", fontname="Palatino Linotype", fontsize=12)
    ax.set_ylabel("ground truth",fontname="Palatino Linotype", fontsize=12 )
...
# Define grid layout
    ...
    for i in np.arange(0, ncols):
        ...
        axes[3*i+1].imshow(gt_list[i], cmap=TUM_CI_colors.sem_seg_cmap, clim=[0, len(label_classes)+1])
        axes[3*i+2].imshow(pr_list[i], cmap=TUM_CI_colors.sem_seg_cmap, clim=[0, len(label_classes)+1])
