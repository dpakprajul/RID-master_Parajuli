...
from definitions import DIR_RESULTS_TRAINING, MODEL_NAME
...
def create_filter_dataset(DATA_DIR_ANNOTATION_EXPERIMENT, DIR_MASK_FILES, DATA_VERSION, label_classes,
    ...
    filter_dataset = Dataset(
        ...
        y_filter_dir,
        filter_filenames,
        classes=[label_classes[1]], #work around to only get pixels with value 0
...
def get_image_gt_and_pr_masks(model, id, test_dataset, filter_dataset, label_classes, filter_to_one_roof_only=True):
    ...
    if filter_to_one_roof_only:
        ...
        gt_vector[filter_vector == 0] = len(label_classes)+1
...
def save_prediction_masks(model, image_dataset, label_classes, DIR_PREDICTIONS1):
    ...
    for count, id in enumerate(np.arange(len(image_dataset))):
        ...
        filepath = os.path.join(DIR_PREDICTIONS1, image_name)
        cv2.imwrite(filepath, predcition)
        # # for debugging:
        #filepath = os.path.join(DIR_PREDICTIONS1 + '_visible', image_name)
...
def visualize_top_median_bottom_predictions_and_ground_truth(model, id_top_5, id_median_5, id_bottom_5, test_dataset, filter_dataset, label_classes):
    ...
    for id in plot_ids:
        # get mask and prediction
        image, gt_vector, pr_vector = get_image_gt_and_pr_masks(model, id, test_dataset, filter_dataset, label_classes, filter_to_one_roof_only=False)
